...
export class CouponOfflineDetailsComponent implements OnInit, OnDestroy {
  ...
  openModal(template: TemplateRef<any>) {
    ...
    if (this.couponPass.CouponTokens.length === 0) {
      this.toastr.error('Hai già venduto tutti i token per questo coupon, oppure non sono più validi.', 'Token terminati');
  ...
  sell() {
    ...
    this.couponService.buyProducerTokensOfflineByToken(this.couponPass.CouponTokens[0].token, this.couponPass.id)
      .subscribe(result => {
        ...
        if (result) {
          ...
          this.toastr.error('I token relativi a questo Coupon sono terminati', 'Token non venduto');
